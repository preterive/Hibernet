	...
	try:
		...
		for proxy in part:
			...
			try:
				...
				pass
		out_file = open("proxies.txt","a")      # e li scrive nel file, aperto come a, per non sovrascrivere proxy gia presenti all'interno
		...
		for tag in soup.find_all(word2, word): # bs4, dopo aver processato la source, trova la parte riguardante le proxylist
			...
			for line in result :
				...
				if ip: # se ha trovato gli ip prosegue
					for x in ip:
						out_file = open("proxies.txt","a") # scrittura singolo ip nella proxy.txt
						while True:
							out_file.write(x+"\n") # scrive ip uno per uno nel file proxy.txt
							out_file.close()
...
def proxylist(): # funzione per la creazione della proxylist
	...
	proxies = open("proxies.txt").readlines() # la lista txt presenta doppioni, quindi:
	proxiesp = []
	for i in proxies:
		if i not in proxiesp: # se il proxy non è già presente in proxiesp
			proxiesp.append(i) # li aggiunge in proxiesp
	filepr = open("proxies.txt", "w") # prima cancella contenuto del file
	filepr.close()
	filepr = open("proxies.txt", "a") # dopo lo apre in modalità a per non sovrascrivere i proxy
	for i in proxiesp:
